...
def _map_section(data,typeQ) :
    ...
    for i in range(len(data['quests'])):
        if data['quests'][i]['type'] == "Socio-démographique" :
            try : 
                time =  data['quests'][i]['datedue']
    ...
    for i in range(len(data['quests'])):
        ...
        try :
            ...
            if data['quests'][i]['type'] == typeQ and date_time >= date_time_2:
                liste.append(i)
                return liste
        except :
            pass
...
def _construct(data,question,parent,user='',questionnaire_type = 'None',ordre=-1,num=0):
    """
    ...
    for element in root:
        ...
        if typeQ == "Range":
            ...
            if 'sousquest' in element['options'] :
                ...
                for subquestion in element['options']['sousquest'] :
                    _construct([subquestion],question,parent,user,ordre=ordre)
        ...
        if typeQ == "Radio" :
            parent.append(element['code'])
            for index,subquestion in enumerate(element['questionOptions']):
                ...
                question[user][typeQ][element['code']][subquestion['text']] = { 
                    ...
                    'required':required,
                    'ordre':ordre}
                if index != len(element['questionOptions'])-1 :
                    ordre += 1
    ...
    for file in files :
        with open("json/{}".format(file), 'r',encoding='utf8') as f:
            ...
            if "quests" in datastore :
                    if datastore["quests"] != []:
                        num = _map_section(datastore,questionnaire_type)
                        for el in num :
                            question = _construct(data = datastore,question = question,parent = ['root'],questionnaire_type = questionnaire_type,num=el)
    return question
    ...
    for key in question.keys():
        if querry in question[key] : 
            for el in question[key][querry]:
                ...
                mm[el] = {}
                mm[el]['min'] = question[key][querry][el]['min-max'][0]
                mm[el]['max'] = question[key][querry][el]['min-max'][1]
...
def pourcentage_rep(question_list):
        ...
        for personne in question_list.keys() :
            ...
            reponse[personne] = tmp_rep/tmp_total
        return reponse 
...
def niveau_ids_unique(question_list,hierarchy):
    liste =  {}
    for personne in question_list.keys() :
        liste[personne] = {}            
        for question in question_list[personne]['Range'] :
            if question in hierarchy and 'value' in question_list[personne]['Range'][question] and question_list[personne]['Range'][question]['value'] != 0:
                liste[personne][question] = question_list[personne]['Range'][question]['value']
    return liste


def niveau_ids_total(question_list,hierarchy,liste={}):
    for personne in question_list.keys() :
        for question in question_list[personne]['Range'] :
            if question not in liste :
                liste[question] = []
            if question in hierarchy and 'value' in question_list[personne]['Range'][question] and  question_list[personne]['Range'][question]['value'] != 0:
                liste[question].append( question_list[personne]['Range'][question]['value'] )
    liste_mean = {}
